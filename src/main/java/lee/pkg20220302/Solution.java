...
//传入某个位置的数字，长度是否为偶数的标识，获取回文数。
    ...
    public String nearestPalindromic(String n) {
        ...
        //将与长度相关的边界值也纳入考虑，即将「长度为 n - 1 n−1 的回文串最大值（99...9999...99）」和「长度为 n + 1 n+1 的回文串最小值（10...0110...01）」也纳入考虑
        // 10 the case
        // 不是n+1/2 就是n/2
        set.add((long) Math.pow(10, (len - 1)) - 1);
        set.add((long) Math.pow(10, len) + 1);

        //截取传来的字符串的前一半 123->12    1234->12
    ...
    public static void main(String[] args) {
        ...
        var ans = app.nearestPalindromic("1");
