...
class DataStoreDemoActivity : AppCompatActivity() {
    ...
    private fun setupJsonDataStoreDemo() {
        ...
        findViewById<android.widget.Button>(R.id.btnReadUser).setOnClickListener {
            lifecycleScope.launch {
                val userPrefs = dataStoreManager.getUserPreferences()
    ...
    private fun setupMultiTypeDemo() {
        ...
        findViewById<android.widget.Button>(R.id.btnReadMultiType).setOnClickListener {
            lifecycleScope.launch {
                val userData = dataStoreManager.readUserData()
                val appConfig = dataStoreManager.readAppConfiguration()
    ...
    private fun setupVersionControlDemo() {
        val tvVersionResult = findViewById<android.widget.TextView>(R.id.tvVersionResult)
        
        findViewById<android.widget.Button>(R.id.btnShowVersions).setOnClickListener {
            val versions = dataStoreManager.getCurrentVersions()
            val versionText = versions.entries.joinToString("\n") { (key, value) ->
                "$key: v$value"
            }
            tvVersionResult.text = versionText
            log("当前版本信息:\n$versionText")
        }
    }
