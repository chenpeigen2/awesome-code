...
class DataStoreDemoActivity : AppCompatActivity() {
    ...
    private fun setupJsonDataStoreDemo() {
        ...
        findViewById<android.widget.Button>(R.id.btnReadUser).setOnClickListener {
            lifecycleScope.launch {
                val userPrefs =
                    dataStoreManager.getObject(DemoConfig.Keys.USER_DATA_JSON, UserPreferences())
                ...
                withContext(Dispatchers.Main) {
                    tvUserResult.text =
                        "用户: ${userPrefs.userName}, 年龄: ${userPrefs.userAge}, 登录时间: $timeStr"
                }
                ...
                withContext(Dispatchers.Main) {
                    if (result.isSuccess) {
                        ...
                        tvTransactionResult.text =
                            "事务执行失败: ${result.exceptionOrNull()?.message}"
